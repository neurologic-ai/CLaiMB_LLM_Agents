...
def _as_float(x, default=None):
    try:
        # tolerate "4", "4.0", Decimal, numpy types, etc.
        return float(x)
    except Exception:
        return default

def _coerce_score(score: Any) -> float | None:
    """Coerce score to float (0..5). Returns None if not coercible."""
    val = _as_float(score, default=None)
    if val is None:
        return None
    # optional: clamp to 0..5 so bad generations don't explode the validator
    if val < 0.0:
        val = 0.0
    if val > 5.0:
        val = 5.0
...
def _finalize_output(metric_name: str, m_in: MetricInput, out: Dict[str, Any]) -> Dict[str, Any]:
    ...
    out.setdefault("category", _NAME_TO_CATEGORY.get(metric_name, Category.efficiency))

    # Score / confidence normalization (accept strings too)
    if "score" in out:
        coerced = _coerce_score(out["score"])
        # if coercion failed, default to 0.0 (let your schema validator catch if needed)
        out["score"] = 0.0 if coerced is None else coerced
    else:
        out["score"] = 0.0

    out["confidence"] = _as_float(out.get("confidence", 0.8), 0.8)
    if out["confidence"] < 0.0:
        out["confidence"] = 0.0
    if out["confidence"] > 1.0:
        out["confidence"] = 1.0

    # Rationale always string
    out["rationale"] = str(out.get("rationale", "")).strip()

    # --- New schema fields we keep ---
    # evidence: ensure dict
    ev = out.get("evidence")
    if not isinstance(ev, dict):
        ev = {}
    out["evidence"] = ev

    # gaps: ensure list[str]
    gaps = out.get("gaps")
    if isinstance(gaps, str):
        gaps = [gaps]
    if not isinstance(gaps, list):
        gaps = []
    out["gaps"] = [str(g).strip() for g in gaps if str(g).strip()]
